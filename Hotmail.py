# DECODE BY:- Abdullha_404
# COPYRIGHT:- @Abdullha_404
...
######AX#####
print(X + 'DeCoDeD By Abdullha_404 : @Xylon1590' + '\n\n' + '\033[0m')
nnn = random.choice([R,X,F,B,K,V])
good_hot,bad_hot,good_ig,bad_ig,check,mj,ids=0,0,0,0,0,0,[]
tok = input('• {}BOT{} ♪ {}TOKEN : {}'.format(B,C,V,K))
print("\r")
iD = input('• {}ID{} ♪ {}BOT : {}'.format(B,C,V,K))
os.system('clear')
def cookie(email):
...
""");hunt = ("""
Yeni Hunt Düştü Bol Şanslar Dostum !
...
def check_email(email):
	...
	bo = f'\x1b[38;5;{b}m'
	check+=1
	sys.stdout.write(f"\r   {bo}[ {C}QNB {bo}] {C}Hit : {F}{good_hot}  {C}Kötü Mail : {R}{bad_ig}  {C}Güzel IG : {X}{good_ig}  {C}{bo}Denetlenen•{check}\r")
	sys.stdout.flush()
